...
async function main() {
  ...
  r = await call('POST', TPORTS.gateway, `/sims/${SIM}/field/actions?branch=main`, {
    ...
    actions: [
      ...
      { clientActionId: 'rp-early', kind: 'reportRepair', teamId: TEAM, orderId: order.id, stage: 'finish', used: { personnel: 6 }, at: '13:15', hlc: hlc(t0 + 13500) },
  ...
  assert(rr[2].ok === false && String(rr[2].msg || '').includes('100%'), '进度未满 100% 的完工上报被前置校验拒绝（快速失败），不阻塞后续动作')
